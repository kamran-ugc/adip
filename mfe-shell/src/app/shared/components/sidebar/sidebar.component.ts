import { Component, Input, ViewChild, ElementRef, AfterViewInit, OnDestroy, Renderer2, OnInit } from "@angular/core";
...
export class SidebarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  toggleSidebar() {
    ...
    if (this.isMinimized) {
      ...
      document.documentElement.style.setProperty('--sidebar-width', `${maxWidthValue}px`);
      
      // Apply max width to ALL elements that might control width
      if (this.sidebarElement && this.sidebarElement.nativeElement) {
        this.renderer.setStyle(this.sidebarElement.nativeElement, 'width', `${maxWidthValue}px`);
      }
  ...
  onResize(event: MouseEvent) {
    ...
    if (newWidth >= 200 && newWidth <= this.maxWidth) {
      ...
      this.renderer.setStyle(this.sidebarElement.nativeElement, 'width', `${newWidth}px`);
